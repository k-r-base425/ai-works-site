...
export function RpgHomeHero() {
  return (
    <section className="relative overflow-hidden rounded-sm border-4 border-[color:var(--rpg-gold)] bg-[color:var(--rpg-bg)] text-[#fff9df] shadow-[var(--rpg-shadow)]">
      ...
      <div className="relative z-10">
        <header className="hidden items-center justify-between border-b-2 border-[color:var(--rpg-gold)]/70 px-8 py-5 lg:flex">
          ...
          <nav className="flex items-center gap-7 text-sm font-black text-[#f8edd0]" aria-label="RPG top navigation">
            {navItems.map((item, index) => (
              <Link key={item.href} to={item.href} className="relative flex items-center gap-2 transition hover:text-[color:var(--rpg-gold-light)]">
                ...
                {item.label}
                {index === 0 ? <span className="absolute -bottom-6 left-1/2 h-1 w-16 -translate-x-1/2 bg-[color:var(--rpg-gold-light)] shadow-[0_0_14px_var(--rpg-gold-light)]" /> : null}
              </Link>
            ))}
          </nav>
        ...
        <div className="grid gap-4 border-b-2 border-[color:var(--rpg-gold)]/60 p-4 sm:p-6 lg:grid-cols-[42%_1fr] lg:p-8">
          <div className="relative rounded-sm border-2 border-[color:var(--rpg-gold)] bg-[color:var(--rpg-panel)] p-5 shadow-[var(--rpg-shadow)] sm:p-7">
            ...
            <p className="mt-6 text-xl font-black leading-9 tracking-normal text-white sm:text-2xl">
              小規模事業者のための
            </p>
            <h1 className="mt-3 text-[2.7rem] font-black leading-tight tracking-normal text-white sm:text-[3.5rem] lg:text-[3.35rem]">
              <span className="mr-3 text-[color:var(--rpg-cyan)] drop-shadow-[0_4px_0_rgba(0,0,0,0.5)]">AI</span>
              制作・活用
              <span className="block text-[2.35rem] sm:text-[3rem] lg:text-[2.75rem]">サポートパートナー</span>
            </h1>

            <div className="mt-6 grid grid-cols-2 border border-[color:var(--rpg-gold)]/50 sm:grid-cols-4">
              {serviceItems.map((item) => (
                <div key={item.label} className="grid min-h-[5rem] place-items-center gap-1 border border-[color:var(--rpg-gold)]/35 bg-[color:var(--rpg-bg-deep)] px-2 py-3 text-center text-xs font-black">
            ...
            <div className="mt-5 grid grid-cols-2 border border-[color:var(--rpg-gold)]/45 sm:grid-cols-4">
              {statItems.map((item) => (
        ...
        <div className="px-4 py-5 sm:px-8">
          <div className="mb-5 flex items-center justify-center gap-3 text-[color:var(--rpg-gold-light)]">
