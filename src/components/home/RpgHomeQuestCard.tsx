...
export function RpgHomeQuestCard({ accent, href, icon: Icon, index, title }: RpgHomeQuestCardProps) {
  return (
    <Link
      to={href}
      className={`group relative grid min-h-[8.5rem] grid-cols-[4.5rem_1fr] items-center gap-4 overflow-hidden rounded-sm border-2 border-[color:var(--rpg-gold)] p-4 text-white shadow-[var(--rpg-shadow)] transition hover:-translate-y-1 ${accentClasses[accent]}`}
    >
      ...
      <span className="mt-5 flex h-16 w-16 items-center justify-center rounded-sm border-2 border-[#f2d98a] bg-[#071522] text-[color:var(--rpg-gold-light)] shadow-[inset_0_0_0_2px_rgba(0,0,0,0.35)]">
        <Icon className="h-9 w-9" strokeWidth={1.65} aria-hidden="true" />
      </span>
      <span className="mt-6 min-w-0 text-lg font-black leading-8 tracking-normal sm:text-xl">
        {title}
      </span>
